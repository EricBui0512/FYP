<form class="form-horizontal" method="POST" action="{{ URL::to('user/login') }}" accept-charset="UTF-8">
    ...
    <div class="span5 duc">                            
            ...
            <input type="text" class="input-block-level" placeholder="{{ Lang::get('confide::confide.username_e_mail') }}" name="email" id="email" value="{{ Input::old('email') }}">
            <label>Password</label><a href="forgot" class="pull-right"><i class="icon-question-sign"></i>&nbsp;Forgot Password</a> 
            <input type="password" class="input-block-level" tabindex="2" placeholder="{{ Lang::get('confide::confide.password') }}" name="password" id="password">
